...
import { ROMAN_PROVINCES, type RomanProvince } from "../../constants/provinces"
import { 
  useMapData, 
  useMapConfiguration, 
  useMapEvents as useMapEventHandlers,
  useEmpireLayerState,
...
export const Map: React.FC<MapProps> = ({
  ...
  const { provincesData, provincesLabelsData, loading: provincesLoading, error } = useMapData()
  ...
  const selectedProvinces = externalSelectedProvinces ?? internalSelectedProvinces
  const showProvinceLabels = externalShowProvinceLabels ?? internalShowProvinceLabels
  const setSelectedProvinces = isExternallyControlled
    ? () => { /* controlled externally */ }
    : setInternalSelectedProvinces
  const setShowProvinceLabels = externalShowProvinceLabels !== undefined
    ? () => { /* controlled externally */ }
  ...
  useEffect(() => {
    Object.entries(empireLayerConfig).forEach(([key, config]) => {
      if (config.showProp !== undefined && isLayerVisible(key) !== config.showProp) {
        toggleLayer(key)
      }
    })
  }, [showBC60, showAD14, showAD69, showAD117, showAD200, empireLayerConfig, isLayerVisible, toggleLayer])
