...
export function EditSomnusView() {
  ...
  const handleFieldChange = (id: string, field: keyof SomnusItem, value: string) => {
    setItems(prev => prev.map(item => 
      item.id === id 
    ...
    try {
      ...
      if (result.success) {
        setItems(prev => prev.map(i => 
          i.id === id 
            ? { ...i, isEditing: false, hasChanges: false }
            : i
        ))
  ...
  const filteredItems = items.filter(item => {
    ...
    return item.nickname?.toLowerCase().includes(nicknameFilter.toLowerCase()) ?? false
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="artemis-card p-4">
        <label htmlFor="nickname-filter" className="block text-sm font-medium mb-2">
          Filter by Nickname
        </label>
        <input
          id="nickname-filter"
          ...
          className="w-full max-w-md px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 focus:border-amber-500"
        />
        {nicknameFilter && (
      ...
      {filteredItems.length === 0 && nicknameFilter ? (
        <div className="artemis-card p-8 text-center">
          ...
          <p className="coin-description">No items found matching &ldquo;{nicknameFilter}&rdquo;</p>
        </div>
      ) : (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left p-3 font-semibold">Legend O</th>
                <th className="text-left p-3 font-semibold">Legend O Expanded</th>
                <th className="text-left p-3 font-semibold">Legend R</th>
                <th className="text-left p-3 font-semibold">Legend R Expanded</th>
                <th className="text-left p-3 font-semibold">Flavour Text</th>
                  ...
                  <td className="p-3">
                    {item.isEditing ? (
                      <input
                        type="text"
                        value={item.legend_o ?? ""}
                        onChange={(e) => handleFieldChange(item.id, "legend_o", e.target.value)}
                        className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                  ...
                  <td className="p-3">
                    {item.isEditing ? (
                      <textarea
                        ...
                        onChange={(e) => handleFieldChange(item.id, "legend_o_expanded", e.target.value)}
                        className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                        placeholder="Legend O Expanded"
                        rows={2}
                      />
                    ) : (
                      <span className="text-sm">{item.legend_o_expanded ?? "—"}</span>
                    )}
                  </td>
                  <td className="p-3">
                    {item.isEditing ? (
                      <input
                        type="text"
                        value={item.legend_r ?? ""}
                        onChange={(e) => handleFieldChange(item.id, "legend_r", e.target.value)}
                        className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                  ...
                  <td className="p-3">
                    {item.isEditing ? (
                      <textarea
                        ...
                        onChange={(e) => handleFieldChange(item.id, "legend_r_expanded", e.target.value)}
                        className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                        placeholder="Legend R Expanded"
                        rows={2}
                      />
                    ) : (
                      <span className="text-sm">{item.legend_r_expanded ?? "—"}</span>
                    )}
                  </td>
                  <td className="p-3">
                    {item.isEditing ? (
                      <textarea
                        ...
                        onChange={(e) => handleFieldChange(item.id, "flavour_text", e.target.value)}
                        className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                        placeholder="Flavour Text"
                        rows={2}
                      />
                    ) : (
                      <span className="text-sm">{item.flavour_text ?? "—"}</span>
                    )}
                  </td>
                  <td className="p-3">
