...
export function MobileDrawer({
  ...
  return (
    ...
    >
      ...
      {/* Drawer content - styled to match popup */}
      <div
        ref={contentRef}
        className={`relative w-full transform rounded-t-2xl border border-slate-600 bg-slate-800 p-4 shadow-xl transition-all duration-300 ease-out ${
          isOpen && !isAnimating
      ...
      >
        ...
        {icon && <div className="absolute top-4 left-4 scale-125">{icon}</div>}

        {/* Close button - absolute positioned in top right */}
        <button
          onClick={onClose}
          className="absolute top-4 right-3 rounded-full p-2 text-slate-400 transition-colors hover:bg-slate-700 hover:text-slate-300"
        ...
        {/* Content */}
        <div className="max-h-[70vh] overflow-y-auto px-4 pb-16">
          {children}
        </div>
      </div>
    </div>
